...
public class AsyncDrawable<T extends View> extends Drawable {
  ...
  private final WeakReference<BitmapLoader.BitmapLoadTask<T>> bitmapLoadTaskReference;

  private final Drawable baseDrawable;

  public AsyncDrawable(Drawable drawable, BitmapLoader.BitmapLoadTask<T> bitmapWorkerTask) {
    if (bitmapWorkerTask == null) {
      throw new IllegalArgumentException("bitmapWorkerTask may not be null");
    }
    baseDrawable = drawable;
    bitmapLoadTaskReference = new WeakReference<BitmapLoader.BitmapLoadTask<T>>(bitmapWorkerTask);
  }

  public BitmapLoader.BitmapLoadTask<T> getBitmapWorkerTask() {
