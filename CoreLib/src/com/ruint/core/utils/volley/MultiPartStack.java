...
public class MultiPartStack extends HurlStack {
  ...
  private static void setMultiPartBody(HttpEntityEnclosingRequestBase httpRequest, Request<?> request)
    ...
    Map<String, File> fileUpload = ((MultiPartRequest) request).getFile();
    for (Map.Entry<String, File> entry : fileUpload.entrySet()) {

      builder.addPart(((String) entry.getKey()), new FileBody((File) entry.getValue()));
    }
    // Iterate the byteArrayBodys
    Map<String, ByteArrayBody> byteMap = ((MultiPartRequest) request).getByte();
    for (Map.Entry<String, ByteArrayBody> entry : byteMap.entrySet()) {
      builder.addPart(((String) entry.getKey()), entry.getValue());
    }
    ContentType contentType = ContentType.create(HTTP.PLAIN_TEXT_TYPE, HTTP.UTF_8);
    // Iterate the stringUploads
    Map<String, String> stringUpload = ((MultiPartRequest) request).getString();
    for (Map.Entry<String, String> entry : stringUpload.entrySet()) {
      try {
        builder.addPart(((String) entry.getKey()), new StringBody((String) entry.getValue(), contentType));
      } catch (Exception e) {
        e.printStackTrace();
      }
    }
