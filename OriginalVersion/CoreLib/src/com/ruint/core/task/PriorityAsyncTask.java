...
public abstract class PriorityAsyncTask<Params, Progress, Result> implements TaskHandler {
   ...
   * caller of this task.
   * <p/>
   * This method can call {@link #publishProgress} to publish updates on the UI
   * thread.
   * 
   * @param params
   ...
   * 
   * @param result
   *          The result, if any, computed in {@link #doInBackground(Object[])},
   *          can be null
   * @see #cancel(boolean)
   * @see #isCancelled()
   */
  ...
  /**
   * <p>
   * Applications should preferably override {@link #onCancelled(Object)}. This
   ...
   * Runnable object. See {@link #execute(Object[])} for more information on the
   * order of execution.
   * 
   * @see #execute(Object[])
   * @see #executeOnExecutor(java.util.concurrent.Executor, Object[])
   ...
   * on the UI thread while the background computation is still running. Each
   * call to this method will trigger the execution of {@link #onProgressUpdate}
   * on the UI thread.
   * <p/>
