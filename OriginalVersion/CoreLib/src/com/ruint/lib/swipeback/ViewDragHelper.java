...
package com.ruint.lib.swipeback;
...
public class ViewDragHelper {
  ...
  /**
   * A view is currently being dragged. The position is currently changing as a
   * result of user input or simulated user input.
   */
  ...
  /**
   * A Callback is used as a communication channel with the ViewDragHelper back
   * to the parent view using it. <code>on*</code>methods are invoked on
   * siginficant events and several accessor methods are expected to provide the
   * ViewDragHelper with more information about the state of the parent view
   * upon request. The callback also makes decisions governing the range and
  ...
  public static abstract class Callback {
    ...
    /**
     * Called when the captured view's position changes as the result of a drag
     * or settle.
     * 
     * @param changedView
    ...
    /**
     * Called when a child view is captured for dragging or settling. The ID of
     * the pointer currently dragging the captured view is supplied. If
     * activePointerId is identified as {@link #INVALID_POINTER} the capture is
    ...
    /**
     * Called when the child view is no longer being actively dragged. The fling
     * velocity is also supplied, if relevant. The velocity values may be
     * clamped to system minimums or maximums.
     * <p>
     * Calling code may decide to fling or otherwise release the view to let it
     * settle into place. It should do so using
     * {@link #settleCapturedViewAt(int, int)} or
     * {@link #flingCapturedView(int, int, int, int)}. If the Callback invokes
     * one of these methods, the ViewDragHelper will enter
     * {@link #STATE_SETTLING} and the view capture will not fully end until it
     * comes to a complete stop. If neither of these methods is invoked before
     * <code>onViewReleased</code> returns, the view will stop in place and the
     * ViewDragHelper will return to {@link #STATE_IDLE}.
     * </p>
     * 
     * @param releasedChild
     *          The captured child view now being released
     * @param xvel
     *          X velocity of the pointer as it left the screen in pixels per
     *          second.
     * @param yvel
     *          Y velocity of the pointer as it left the screen in pixels per
     *          second.
     */
     ...
     * 
     * @param edgeFlags
     *          A combination of edge flags describing the edge(s) currently
     *          touched
     * @param pointerId
    ...
    /**
     * Called when the given edge may become locked. This can happen if an edge
     * drag was preliminarily rejected before beginning, but after
     * {@link #onEdgeTouched(int, int)} was called. This method should return
     * true to lock this edge or false to leave it unlocked. The default
     * behavior is to leave edges unlocked.
     * 
     * @param edgeFlags
    ...
    /**
     * Called when the user has started a deliberate drag away from one of the
     * subscribed edges in the parent view while no child view is currently
     * captured.
     * 
     * @param edgeFlags
     ...
     * motion in pixels. This method should return 0 for views that cannot move
     * horizontally.
     * 
     * @param child
     *          Child view to check
    ...
    /**
     * Return the magnitude of a draggable child view's vertical range of motion
     * in pixels. This method should return 0 for views that cannot move
     * vertically.
     * 
     * @param child
     *          Child view to check
     ...
     * given child view with the pointer indicated by pointerId. The callback
     * should return true if the user is permitted to drag the given view with
     * the indicated pointer.
     * <p>
     * ViewDragHelper may call this method multiple times for the same view even
     * if the view is already captured; this indicates that a new pointer is
     * trying to take control of the view.
     * </p>
     * <p>
     ...
     * The default implementation does not allow horizontal motion; the
   ...
   *          Parent view to monitor
   * @param sensitivity
   *          Multiplier for how sensitive the helper should be about detecting
   *          the start of a drag. Larger values are more sensitive. 1.0f is
   *          normal.
   * @param cb
   *          Callback to provide information and receive events
   * @return a new ViewDragHelper instance
   ...
   * than zero in pixels per second. Callback methods accepting a velocity will
   ...
   * callback's
   * {@link com.ruint.lib.swipeback.ViewDragHelper.Callback#onEdgeTouched(int, int)}
   * and
   * {@link com.ruint.lib.swipeback.ViewDragHelper.Callback#onEdgeDragStarted(int, int)}
   ...
   * this view that will actively detect edge touches or drags if edge tracking
   * is enabled.
   * 
  ...
  /**
   * Capture a specific child view for dragging within the parent. The callback
   * will be notified but
   * {@link com.ruint.lib.swipeback.ViewDragHelper.Callback#tryCaptureView(android.view.View, int)}
   ...
   * {@link #continueSettling(boolean)} on each subsequent frame to continue the
   * motion until it returns false. If this method returns false there is no
  ...
  /**
   * Settle the captured view at the given (left, top) position. The appropriate
   * velocity from prior motion will be taken into account. If this method
   * returns true, the caller should invoke {@link #continueSettling(boolean)}
   * on each subsequent frame to continue the motion until it returns false. If
   * this method returns false there is no further work to do to complete the
   * movement.
   * 
   * @param finalLeft
   ...
   * the minimum, it will be clamped to zero. If the value is above the maximum,
   * it will be clamped to the maximum.
   * 
   * @param value
   *          Value to clamp
   * @param absMin
  ...
  /**
   * Settle the captured view based on standard free-moving fling behavior. The
   * caller should invoke {@link #continueSettling(boolean)} on each subsequent
   ...
   * time. If <code>continueSettling</code> returns true, the caller should call
   * it again on the next frame to continue.
   * 
   * @param deferCallbacks
   *          true if state callbacks should be deferred via posted message. Set
   *          this to true if you are calling this method from
   ...
   * involves some extra semantics. During a release (mReleaseInProgress) is the
   * only time it is valid to call {@link #settleCapturedViewAt(int, int)} or
  ...
  /**
   * Attempt to capture the view with the given pointer ID. The callback will be
   * involved. This will put us into the "dragging" state. If we've already
  ...
  /**
   * Check if this event as provided to the parent view's onInterceptTouchEvent
   * should cause the parent to intercept the touch event stream.
   * 
   * @param ev
  ...
  /**
   * Check if we've crossed a reasonable touch slop for the given child view. If
   * the child cannot be dragged along the horizontal or vertical axis, motion
   ...
   * {@link #shouldInterceptTouchEvent(android.view.MotionEvent)} or
   * {@link #processTouchEvent(android.view.MotionEvent)}. You should only rely
   * on the results of this method after all currently available touch data has
   * been provided to one of these two methods.
   * </p>
   * 
   * @param directions
   *          Combination of direction flags, see {@link #DIRECTION_HORIZONTAL},
  ...
  /**
   * Check if any of the edges specified were initially touched in the currently
   * active gesture. If there is no currently active gesture this method will
   * return false.
   * 
   * @param edges
   *          Edges to check for an initial edge touch. See {@link #EDGE_LEFT},
   ...
   * with the specified ID. If there is no currently active gesture or if there
   * is no pointer with the given ID currently down this method will return
   * false.
   * 
   * @param edges
   *          Edges to check for an initial edge touch. See {@link #EDGE_LEFT},
  ...
  private void releaseViewForPointerUp() {
    ...
    final float xvel = clampMag(VelocityTrackerCompat.getXVelocity(mVelocityTracker, mActivePointerId), mMinVelocity,
        mMaxVelocity);
    final float yvel = clampMag(VelocityTrackerCompat.getYVelocity(mVelocityTracker, mActivePointerId), mMinVelocity,
        mMaxVelocity);
   ...
   * {@link com.ruint.lib.swipeback.ViewDragHelper.Callback#getOrderedChildIndex(int)}
   * .
   * 
   * @param x
   *          X position to test in the parent's coordinate system
